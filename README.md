...
на сервисе
- `DATABASE_URL` - postgresql://your_user:your_password@your_host:your_port/name_db
- `ENVIRONMENT` - не обходимо задать окружение в котором находится проект, по умолчанию стоит
'development'

Обновить проект на сервере можно используя скрипт deploy.sh. После запуска скрипта применятся обновления из git-а,
обновятся зависимости, перезагрузятся сервисы... Команда для запуска скрипта.
```bash
./deploy.sh
```

Ознакомиться с рабочей версией сайта можно [здесь](https://aleksashka.info/)
